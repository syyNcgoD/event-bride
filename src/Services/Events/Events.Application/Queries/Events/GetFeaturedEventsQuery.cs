    ...
    : IRequestHandler<GetFeaturedEventsQuery, ApiResponse<List<EventSummaryResponse>>>
{
    ...
    public GetFeaturedEventsQueryHandler(IEventRepository eventRepository, ICacheService cacheService)
    {
    ...
    {
        var key = $"{CacheKeys.FeaturedEvents}:{request.Count}";

        var items = await _cacheService.GetOrSetAsync(key, async () =>
        {
